...
void Bot_Think( CLabPlayer *pBot );
...
CBasePlayer *BotPutInServer( bool bFrozen, int iTeam )
{
	...
	CLabPlayer *pPlayer = ((CLabPlayer *)CBaseEntity::Instance( pEdict ));
	pPlayer->ClearFlags();
...
void Bot_RunAll( void )
{
	...
	{
		CLabPlayer *pPlayer = ToLabPlayer( UTIL_PlayerByIndex( i ) );
...
// Output : 	virtual void
//-----------------------------------------------------------------------------
static void RunPlayerMove( CLabPlayer *fakeclient, const QAngle& viewangles, float forwardmove, float sidemove, float upmove, unsigned short buttons, byte impulse, float frametime )
{
	if ( !fakeclient )
		return;
